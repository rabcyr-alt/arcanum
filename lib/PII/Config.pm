...
PII::Config - Configuration loader and validator for pii-guardian
...
Loads pii-guardian config from the first matching location in the search
...
my @CONFIG_SEARCH = (
    '.pii-guardian.jsonc',
    "$ENV{HOME}/.config/pii-guardian/config.jsonc",
    '/etc/pii-guardian/config.jsonc',
);
...
sub _load_default {
    ...
    my $module_file = $INC{'PII/Config.pm'} // __FILE__;
    my $module_dir  = Path::Tiny::path($module_file)->parent->parent->parent;
...
sub _load_profile {
    ...
    # 2. ~/.config/pii-guardian/profiles/
    push @search_dirs, Path::Tiny::path($ENV{HOME}, '.config', 'pii-guardian', 'profiles');
