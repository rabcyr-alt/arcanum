...
PII::Tombstone - Tombstone database for pii-guardian

=head1 SYNOPSIS

    # Build an index from one or more scan roots at scan startup
    my $ts = PII::Tombstone->new(
...
Manages the C<.pii-guardian-tombstones> JSON-Lines file that records SHA-256
hashes of files deleted by pii-guardian remediation.

During each scan, pii-guardian loads tombstone entries from the scan roots and
...
Append a tombstone entry to the appropriate C<.pii-guardian-tombstones> file
...
sub write {
    ...
    # Append to file
    my $ts_file = path($root)->child('.pii-guardian-tombstones');
