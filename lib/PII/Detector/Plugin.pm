...
PII::Detector::Plugin - External subprocess plugin bridge for pii-guardian
...
Invokes an external plugin executable (any language) using the pii-guardian
...
=item 2. C<~/.config/pii-guardian/plugins/>
...
Extra args beyond C<PII::Detector::Base::new>:
...
sub new {
    ...
    my $plugin_name = $args{plugin_name}
        or die "PII::Detector::Plugin: plugin_name required\n";
...
2. C<~/.config/pii-guardian/plugins/$name> (with same extension variants)
...
sub find_plugin_executable {
    ...
    my @dirs    = (
        "$config_dir/plugins",
        "$home/.config/pii-guardian/plugins",
    );
...
sub _framework_tags {
    my ($self, $type) = @_;
    require PII::Report::ComplianceMap;
    return [ PII::Report::ComplianceMap->framework_tags_for($type) ];
}
