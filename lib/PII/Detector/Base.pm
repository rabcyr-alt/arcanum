...
PII::Detector::Base - Abstract base class for pii-guardian detectors

=head1 SYNOPSIS

    package PII::Detector::MyType;
...
The full effective config hashref from C<PII::Config>.
...
sub new {
    ...
    croak "PII::Detector::Base is abstract; subclass it and implement detector_type() and detect()"
        if $class eq 'PII::Detector::Base';
