...
PII::Remediation::Base - Shared base for pii-guardian remediation modules
...
=item * Audit log writer (JSON Lines to C<.pii-guardian-audit.jsonl>)

=item * Tombstone writer/reader (C<.pii-guardian-tombstones>)
...
sub new {
    ...
    croak "PII::Remediation::Base is abstract" if $class eq 'PII::Remediation::Base';
...
sub audit_log {
    ...
    my $log_path = Path::Tiny->new($self->{scan_root})
                             ->child('.pii-guardian-audit.jsonl');
...
Return an arrayref of tombstone entries loaded from C<.pii-guardian-tombstones>.
...
Copy C<$path> to C<$path.pii-guardian-backup-YYYYMMDDHHMMSS>.
...
sub backup_file {
    ...
    my $bak     = "${path}.pii-guardian-backup-${ts}";
