package App::Arcanum::FileClassifier;
...
App::Arcanum::FileClassifier - Walk directories and classify files for arcanum

=head1 SYNOPSIS

    my $fc = App::Arcanum::FileClassifier->new(config => $cfg, logger => $log);
...
sub _load_tombstones {
    ...
    my $cfg           = $self->{config};
    my $tombstone_file = $cfg->{report}{tombstone_file} // '.arcanum-tombstones';
