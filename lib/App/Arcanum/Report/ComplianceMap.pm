package App::Arcanum::Report::ComplianceMap;
...
App::Arcanum::Report::ComplianceMap - Regulatory framework compliance report for arcanum

=head1 SYNOPSIS

    my $cm = App::Arcanum::Report::ComplianceMap->new(config => $cfg, logger => $log);
...
produced by arcanum's detector suite.  Output includes:
...
#
# Keys: finding type (as produced by App::Arcanum::Detector::*)
...
Used by detectors to annotate findings with regulatory context.
    ...
    my @tags = App::Arcanum::Report::ComplianceMap->framework_tags_for('credit_card');
    # => ('pci_dss')
