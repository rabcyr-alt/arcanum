package App::Arcanum::Notification::GitHub;
...
App::Arcanum::Notification::GitHub - GitHub issue/comment notification backend
...
sub send {
    ...
    if ($action eq 'issue') {
        ...
        $post = { title => $title, body => $body, labels => ['arcanum'] };
    }
    else {
