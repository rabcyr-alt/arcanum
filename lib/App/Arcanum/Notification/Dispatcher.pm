package App::Arcanum::Notification::Dispatcher;
...
App::Arcanum::Notification::Dispatcher - Fan-out notification delivery for arcanum
...
sub _build_payload {
    ...
    return {
        subject            => "arcanum: PII detected — $summary",
        summary            => $summary,
...
sub _enabled_backends {
    my ($self) = @_;
    my @all = (
        App::Arcanum::Notification::Email->new(     config => $self->{config}, logger => $self->{logger} ),
        App::Arcanum::Notification::Webhook->new(   config => $self->{config}, logger => $self->{logger} ),
        App::Arcanum::Notification::GitHub->new(    config => $self->{config}, logger => $self->{logger} ),
        App::Arcanum::Notification::GitLab->new(    config => $self->{config}, logger => $self->{logger} ),
        App::Arcanum::Notification::Bitbucket->new( config => $self->{config}, logger => $self->{logger} ),
    );
