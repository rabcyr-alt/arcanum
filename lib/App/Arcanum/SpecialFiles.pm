package App::Arcanum::SpecialFiles;
...
App::Arcanum::SpecialFiles - Special file type detection and scanning for arcanum

=head1 SYNOPSIS

    my $sf = App::Arcanum::SpecialFiles->new(config => $cfg, logger => $log);
...
Apply all detectors plus `App::Arcanum::Detector::CommandLinePII`.
...
sub _cli_detector {
    ...
    unless ($self->{_cli_det}) {
        require App::Arcanum::Detector::CommandLinePII;
        $self->{_cli_det} = App::Arcanum::Detector::CommandLinePII->new(
            config => $self->{config},
            logger => $self->{logger},
        );
    }
